...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/supply_demand.log'),
...
try:
    logger.info('수급 데이터 수집 시작')
    
    config = load_config()
    db = init_db(config)
    
    collector = SupplyDemandCollector(config, db)
